...
class UclidProcedureSig(UclidElement):
    ...
    def __init__(
        ...
        Args:
            inputs (List[(UclidLiteral|str, UclidType)]): List of (typed) input
                arguments
            modifies (List[UclidLiteral], optional): List of modified variables.
                Defaults to None.
            returns (List[(UclidLiteral|str, UclidType)], optional): List of returned
                variables. Defaults to None.
            requires (UclidExpr, optional): Input/initial assumptions in procedural
...
class UclidInstanceVarAccess(UclidExpr):
    def __init__(self, instance: UclidInstance, var: UclidLiteral):
        ...
        Args:
            ...
            var (UclidExpr): Variable to access (must be a single variable, not an
                expression)
        """
        self.instance = (
...
class UclidCaseStmt(UclidStmt):
    def __init__(self, conditionlist: List[UclidExpr], stmtlist: List[UclidStmt]):
        ...
        Args:
            ...
            stmtlist (List[UclidStmt]): List of statements to execute based on
                the conditions
        """
        if not isinstance(conditionlist, list) or not isinstance(stmtlist, list):
            _logger.error(
                "UclidCase requires a pair of lists denoting case-split conditions\
                    and statements"
            )
            _logger.error(
...
class UclidITENestedStmt(UclidStmt):
    def __init__(self, conditionlist: List[UclidExpr], stmtlist: List[UclidStmt]):
        ...
        Args:
            conditionlist (List[UclidExpr]): List of conditions (each nested within
                the previous else block)
...
class UclidHavocStmt(UclidStmt):
    def __init__(self, variable: UclidVar):
        """Havoc statement: this sets the variable to an arbitrary (non-deterministic)
            value
...
class UclidPrintCexCommand(UclidControlCommand):
    def __init__(self, enginename: str, trace_items: List[UclidExpr] = []):
        ...
        Args:
            ...
            trace_items (List[UclidExpr], optional): Variables to print from the
                counterexample trace. Defaults to [] (in which case all module
...
class UclidPrintCexJSONCommand(UclidControlCommand):
    def __init__(self, enginename: str, trace_items: List[UclidExpr] = []):
        ...
        Args:
            enginename (str): Name of proof object
            trace_items (List[UclidExpr], optional): Variables to print from the
                counterexample trace. Defaults to [] (in which case all module variables
                are printed).
        """
        super().__init__()
        self.enginename = enginename
...
class UclidControlBlock(UclidElement):
    def __init__(self, stmts: List[UclidControlCommand] = []):
        ...
        Args:
            stmts (List[UclidControlCommand], optional): Commands to execute. Defaults
                to [].
        """
        super().__init__()
        self.stmts = stmts
...
class UclidModule(UclidElement):
    ...
    def __init__(
        ...
        Args:
            ...
            _init (UclidInitBlock, optional): Init block. Defaults to
                UclidInitBlock([]).
            _next (UclidNextBlock, optional): next block. Defaults to
                UclidNextBlock([]).
            _control (UclidControlBlock, optional): control block. Defaults to
                UclidControlBlock([]).
        """
        super().__init__()
        # Module elements
    ...
    def mkArrayType(self, *args) -> UclidType:
        ...
        Args:
            *args:
                ...
                - 3: name (str): type synonym, itype (UclidType): index type, etype
                    (UclidType): element type
    ...
    def mkConst(self, name: str, typ: UclidType, value=None) -> UclidConst:
        ...
        Args:
            ...
            value (UclidLiteral|str|int, optional): Value of the constant. Defaults
                to None.
    ...
    def mkInstance(
        ...
        Args:
            ...
            argmap (Dict[str, UclidExpr], optional): mapping from input/output portnames
                of submodule to expressions. Defaults to {}.
    ...
    def mkImport(self, decltype: DeclTypes, name: str, modulename, refname: str):
        ...
        Args:
            decltype (DeclTypes): What kind of declaration is this (e.g. type, function,
                procedure, etc.)?
    ...
    def __inject__(self) -> str:
        # dependencies = [inst.obj.module.name for inst in
        #   UclidContext.instance_decls[self.name].values()]
        # acc = "\n\n".join([UclidContext.modules[dep].__inject__()
