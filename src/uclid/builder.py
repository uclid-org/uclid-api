...
class UclidInitBlock(UclidElement):
    def __init__(self, block) -> None:
        ...
        if isinstance(block, UclidBlockStmt):
            ...
            _logger.error(
                "Unsupported type {} of block in UclidInitBlock".format(type(block))
            )
...
class UclidNextBlock(UclidElement):
    def __init__(self, block) -> None:
        ...
        if isinstance(block, UclidBlockStmt):
            ...
            _logger.error(
                "Unsupported type {} of block in UclidNextBlock".format(type(block))
            )
...
class UclidProcedureSig(UclidElement):
    ...
    def __init__(
        ...
        Args:
            inputs (List[(UclidLiteral|str, UclidType)]):
                List of (typed) input arguments
            modifies (List[UclidLiteral], optional):
                List of modified variables. Defaults to None.
            returns (List[(UclidLiteral|str, UclidType)], optional):
                List of returned variables. Defaults to None.
            requires (UclidExpr, optional):
                Input/initial assumptions in procedural verification. Defaults to None.
            ensures (UclidExpr, optional):
                Output/final guarantees in procedural verification. Defaults to None.
        """
        super().__init__()
        self.inputs = inputs
    ...
    def __inject__(self) -> str:
        input_str = ", ".join(
            [
                "{} : {}".format(
                    i[0] if isinstance(i[0], str) else i[0].lit, i[1].__inject__()
                )
                for i in self.inputs
            ]
        )
        ...
        requires_str = (
            "\nrequires ({});".format(self.requires.__inject__())
            if self.requires is not None
            else ""
        )
        ensures_str = (
            "\nensures ({});".format(self.ensures.__inject__())
...
class UclidFunctionApplication(UclidExpr):
    ...
    def __inject__(self) -> str:
        return "{}({})".format(
            self.function, ", ".join([arg.__inject__() for arg in self.arglist])
        )
...
class UclidArraySelect(UclidExpr):
    def __init__(self, arrayexpr: UclidExpr, indexseq: List[UclidExpr]):
        ...
        super().__init__()
        self.arrayexpr = (
            arrayexpr if isinstance(arrayexpr, str) else arrayexpr.__inject__()
        )
        self.indexseq = [
    ...
    def __inject__(self) -> str:
        return "{}[{}]".format(
            self.arrayexpr, "][".join([ind.__inject__() for ind in self.indexseq])
        )
...
class UclidArrayUpdate(UclidExpr):
    ...
    def __inject__(self) -> str:
        return "{}[{} -> {}]".format(
            self.arrayexpr, self.index.__inject__(), self.value.__inject__()
        )
...
class UclidRecordUpdate(UclidExpr):
    ...
    def __inject__(self) -> str:
        return "{}[{} := {}]".format(
            self.recexpr, self.elemname, self.value.__inject__()
        )
...
class UclidInstanceVarAccess(UclidExpr):
    def __init__(self, instance: UclidInstance, var: UclidLiteral):
        ...
        Args:
            instance (UclidInstance/str):
                Module instance (or its name)
            var (UclidExpr):
                Variable to access (must be a single variable, not an expression)
        """
        self.instance = (
...
class UclidAssignStmt(UclidStmt):
    def __init__(self, lval: UclidExpr, rval: UclidExpr) -> None:
        ...
        if isinstance(rval, UclidExpr):
            ...
            _logger.error(
                f"Unsupported rval {rval} has type {type(rval)} in UclidAssign"
            )
...
class UclidCaseStmt(UclidStmt):
    def __init__(self, conditionlist: List[UclidExpr], stmtlist: List[UclidStmt]):
        ...
        Args:
            conditionlist (List[UclidExpr]):
                List of boolean condition expressions
            stmtlist (List[UclidStmt]):
                List of statements to execute based on the conditions
        """
        if not isinstance(conditionlist, list) or not isinstance(stmtlist, list):
            _logger.error(
                "UclidCase requires a pair of lists denoting "
                "case-split conditions and statements"
            )
            _logger.error(
                "Received {} and {}".format(type(conditionlist), type(stmtlist))
            )
            exit(1)
...
class UclidITENestedStmt(UclidStmt):
    def __init__(self, conditionlist: List[UclidExpr], stmtlist: List[UclidStmt]):
        ...
        Args:
            conditionlist (List[UclidExpr]):
                List of conditions (each nested within the previous else block)
            stmtlist (List[UclidStmt]):
...
class UclidInstanceProcedureCallStmt(UclidStmt):
    def __init__(
        self,
        instance: UclidInstance,
        proc,
        inputs: List[UclidExpr],
        returns: List[UclidExpr],
    ):
...
class UclidHavocStmt(UclidStmt):
    def __init__(self, variable: UclidVar):
        """Havoc statement:
            this sets the variable to an arbitrary (non-deterministic) value
...
class UclidPrintCexCommand(UclidControlCommand):
    def __init__(self, enginename: str, trace_items: List[UclidExpr] = []):
        ...
        Args:
            ...
            trace_items (List[UclidExpr], optional):
                Variables to print from the counterexample trace.
                Defaults to [] (in which case all module variables are printed).
        """
        super().__init__()
...
class UclidControlBlock(UclidElement):
    def __init__(self, stmts: List[UclidControlCommand] = []):
        ...
        Args:
            stmts (List[UclidControlCommand], optional):
                Commands to execute. Defaults to [].
        """
        super().__init__()
        self.stmts = stmts
...
class UclidModule(UclidElement):
    ...
    def __init__(
        ...
        Args:
            ...
            _init (UclidInitBlock, optional):
                Init block. Defaults to UclidInitBlock([]).
            _next (UclidNextBlock, optional):
                next block. Defaults to UclidNextBlock([]).
            _control (UclidControlBlock, optional):
                control block. Defaults to UclidControlBlock([]).
        """
        super().__init__()
        # Module elements
        self.name: str = _name
    ...
    def mkArrayType(self, *args) -> UclidType:
        ...
        Args:
            *args:
                - 2: itype (UclidType):
                    index type, etype (UclidType): element type
                - 3: name (str):
                    type synonym, itype (UclidType): index type,
                    etype (UclidType): element type
        ...
        if len(args) == 2:
            ...
            if name in self.type_decls:
                _logger.warn(
                    "Redeclaration of type named {} in module {}".format(
                        name, self.name
                    )
                )
            else:
    ...
    def mkVar(self, name: str, typ: UclidType, porttype=PortType.var) -> UclidVar:
        ...
        """
        if (
            name in self.var_decls
            or name in self.op_var_decls
            or name in self.ip_var_decls
        ):
            _logger.warn(
    ...
    def mkConst(self, name: str, typ: UclidType, value=None) -> UclidConst:
        ...
        Args:
            ...
            typ (UclidType):
                Constant type
            value (UclidLiteral|str|int, optional):
                Value of the constant. Defaults to None.
        ...
        if name in self.const_decls:
            _logger.warn(
                "Redeclaration of const {} in module {}".format(name, self.name)
            )
        else:
            c = UclidConst(name)
    ...
    def mkInstance(
        self, name: str, module, argmap: Dict[str, UclidExpr] = {}
    ) -> UclidInstance:
        ...
        Args:
            ...
            argmap (Dict[str, UclidExpr], optional):
                mapping from input/output portnames of submodule to expressions.
                Defaults to {}.
        ...
        if name in self.instance_decls:
            _logger.warn(
                "Redeclaration of instance {} in module {}".format(name, self.name)
            )
        else:
    ...
    def mkImport(self, decltype: DeclTypes, name: str, modulename, refname: str):
        ...
        Args:
            decltype (DeclTypes):
                What kind of declaration is this (e.g. type, function, procedure, etc.)?
        ...
        if name in self.import_decls:
            _logger.warn(
                "Redeclaration of import {} in module {}".format(name, self.name)
            )
        else:
    ...
    def mkDefine(
        self, name: str, functionsig: UclidFunctionSig, body: UclidStmt
    ) -> UclidDefine:
        ...
        if name in self.define_decls:
            _logger.error(
                "Redeclaration of define macro {} in module {}".format(name, self.name)
            )
        else:
    ...
    def mkProcedure(
        ...
        if name in self.procedure_defns:
            _logger.error(
                "Redeclaration of procedure {} in module {}".format(name, self.name)
            )
        else:
    ...
    def mkProperty(self, name, body: UclidExpr, is_ltl=False) -> UclidSpecDecl:
        ...
        if name in self.module_properties:
            _logger.error(
                "Redeclaration of property {} in module {}".format(name, self.name)
            )
        else:
    ...
    def __module_properties__(self):
        return "\n".join(
            [
                textwrap.indent(assr.__inject__(), "\t")
    ...
    def __inject__(self) -> str:
        ...
        next_code = textwrap.indent(
            self.next.__inject__() if self.next is not None else "", "\t"
        )
